...
<head>
  ...
  <link rel="icon" type="image/x-icon" href="dist/img/drspine_fav.png">
...
<link rel="stylesheet" href="css/calendar-sidebar.css">
...
#eventDetailsModal .modal-content {
    ...
    padding: 20px;
    position: relative; /* Keeps the modal content within the centered container */
...
.patient-info, .doctor-info, .appointment-details {
    margin-bottom: 10px;
}
...
.appointment-status, .appointment-actions {
    margin-bottom: 10px;
}
...
<body class="hold-transition sidebar-mini layout-fixed">
  <div class="wrapper">
    ...
    <div class="preloader flex-column justify-content-center align-items-center">
      <img class="animation__shake" src="dist/img/drspine_logo.png" alt="" height="60" width="80">
    </div>
...
<div class="row card-body p-0">
  <div class="col-md-3 pl-0" style="border-right: 1px solid #cfcccc;box-shadow: 0 0 1px rgba(0, 0, 0, .125), 0 1px 3px rgba(0, 0, 0, .2);">
              <div class="card" style="box-shadow: none;">
                <div class="card-body">
                  <div class="d-flex justify-content-between">
                    <h2 class="card-title">Consultants</h2>
                    <a href="#" class="ml-auto" id="toggleMore" onclick="toggleDoctors()">..more</a>
                  </div>

                  <input type="search" placeholder="Search Doctor" class="mb-4 mt-2 search_doctor" />
                ...
                COALESCE(COUNT(a.doctor_name), 0) AS appointment_count 
         FROM doctor d 
         LEFT JOIN drspine_appointment a 
         ON a.doctor_name = d.doctor_name COLLATE utf8mb4_general_ci 
         AND DATE(a.appointment_date_time) = CURDATE() 
 ...
 $result = $conn->query($sql);
                    ...
                    if ($result && $result->num_rows > 0) {
                      $count = 0;
                      while ($row = $result->fetch_assoc()) {
                        $displayStyle = $count < 2 ? 'block' : 'none';
                        $circleColor = generateRandomColor(); 

                        echo '<p class="card-text doctor" style="display: ' . $displayStyle . ';">';
                        echo '<span class="circle" style="background-color: ' . $circleColor . ';"></span>';
                        echo $row['doctor_name'] . ' (' . $row['appointment_count'] . ')';
                        echo '</p>';

                        $count++;
                      }
                    } else {
                      echo '<p class="card-text">No doctors found.</p>';
                    }
                    ...
                    <script>
                      var displayAllDoctors = false;

                      function toggleDoctors() {
                        var doctorsContainer = document.getElementById('doctorsContainer');
                        var doctors = doctorsContainer.getElementsByClassName('doctor');

                        if (displayAllDoctors) {
                          for (var i = 2; i < doctors.length; i++) {
                        ...
                        displayAllDoctors = !displayAllDoctors;
                        var toggleMore = document.getElementById('toggleMore');
                        toggleMore.textContent = displayAllDoctors ? '..less' : 'more';
                      }
                    </script>
                    ...
                    <script>
                      $(document).ready(function() {
                        $('.search_doctor').on('keyup', function() {
                          ...
                          $('#doctorsContainer .doctor').each(function() {
                            var doctorName = $(this).text().toLowerCase();
                            var shouldDisplay = displayAllDoctors || doctorName.includes(searchValue);
  ...
  <div class="col-md-6 " id="calendar" style="border-right: 1px solid #cfcccc;box-shadow: 0 0 1px rgba(0, 0, 0, .125), 0 1px 3px rgba(0, 0, 0, .2);">
    <input type="text" id="hiddenInputForDatepicker" style="display:none;">
  </div>
  <!-- <div class=""> -->
    <div id="calendar-right-sidebar" class="col-md-3 calendar-right-sidebar">
      <div>
...
<script>
  ...
  $(document).ready(function () {
    // Modal should only show on click of patient-name-time-wrapper
    $(document).on('click', '.patient-name-time-wrapper', function () {
        var rightsidebar_popup_id = $(this).find('.rightsidebar_popup_id').text();
        console.log("on clicked patient-name-time-wrapper", rightsidebar_popup_id);
        get_patientdetails_with_id(rightsidebar_popup_id);
...
function get_patientdetails_with_id(id) {
    // Fetch the patient details using AJAX or any other method
    // and update the modal content.

    // For demo purposes, we'll use some dummy data:
    $('#popup_id').val(id);
    $('#patientFirstName').text("John");
    $('#patientLastName').text("Doe");
    $('#contactNumber').text("123-456-7890");
    $('#on_hover_gender').text("Male");
    $('#doctorName').text("Dr. Smith");
    $('#appointmentTime').text("Tuesday, August 8, 2024, 10:00 AM");
    $('#span_eventdetailsmodal_duration').text("30 minutes");
    ...
    <div class="modal-content">
        <div id="patient_details_modal" class="patient-details-modal">
            ...
            <div class="patient-info">
                <p class="patient-name"><span id="patientFirstName"></span> <span id="patientLastName"></span></p>
            ...
            <div class="appointment-details">
                ...
                <span id="span_eventdetailsmodal_duration"></span>
...
<div id="cancelAppointmentModal"  class="modal fade show" role="dialog" tabindex="-1" style="display: none; margin-top: 150px;">
  <div class="modal-dialog" role="document">
    <div class="modal-content" >
      <div class="modal-header">
...
<script>
$(document).ready(function () {
    $("#cancelAppointmentButton").click(function (event) {
        ...
        $.ajax({
            ...
            dataType: 'json', // Ensure the server response is expected to be JSON
            error: function (xhr, status, error) {
            ...
            success: function (response) {
                ...
                // Ensure response is properly parsed and handled
...
window.addEventListener("scroll", function () {
  ...
  var modal = $('#cancelAppointmentModal');
    var cancelBtn = $('#cancelBtn');
    var popupIdInput = $('#popup_id');
    var cancelledByIdInput = $('#cancelled_by_id');

    // Show the modal when the cancel button is clicked
    cancelBtn.on("click", function () {
        var cancelled_by_id = popupIdInput.val(); // Get the value from popup_id input
        console.log("Cancelled By ID: " + cancelled_by_id);

        // Assign the value to cancelled_by_id input field
        cancelledByIdInput.val(cancelled_by_id);
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.getElementById("saveEditBtn").addEventListener("click", function(event) {
        ...
        const editPatientFirstName = document.getElementById("editPatientFirstName").value;
        const editPatientLastName = document.getElementById("editPatientLastName").value;

        const edit_branchName = document.getElementById("editeventmodal_branch_name").value;
        ...
        $.ajax({
            ...
            data: {
                ...
                patient_first_name: editPatientFirstName,
                patient_last_name: editPatientLastName
            },
            success: function(data) {
                if (data.success) {
...
document.addEventListener('DOMContentLoaded', function () {
  var duration = $('#span_eventdetailsmodal_duration').text();

    // Set the select value
    $('#editeventmodal_duration_select').val(duration).change();
    ...
    editButton.addEventListener('click', function () {
        ...
        var originalPatientFirstName = document.getElementById('patientFirstName').textContent;
var originalPatientLastName = document.getElementById('patientLastName').textContent;

var originalPatientName = originalPatientFirstName + ' ' + originalPatientLastName;
...
var valueToTransfer = onhover_appointment_date_time.value;
      ...
      var editPatientFirstNameInput = document.getElementById('editPatientFirstName');
if (editPatientFirstNameInput) {
    editPatientFirstNameInput.value = originalPatientFirstName;
}

// Set the value of the editPatientLastName input field
var editPatientLastNameInput = document.getElementById('editPatientLastName');
if (editPatientLastNameInput) {
    editPatientLastNameInput.value = originalPatientLastName;
}
        ...
        <div class="card-body">
            <form>
             ...
             <label for="editPatientFirstName">First Name:</label>
                <input type="text" name="editPatientFirstName" id="editPatientFirstName" class="form-control">
              
              <label for="editPatientLastName">Last Name:</label>
                <input type="text" name="editPatientLastName" id="editPatientLastName" class="form-control">
                
<!-- 
                <label for="editPatient_mobilenumber">Mobile Number:</label>
                <input type="text" name="editeventmodal_mobileNumber" id="editeventmodal_mobileNumber" class="form-control">
                 -->
...
function doSomethingWithSelectedDate(date) {
  ...
  function openAppointmentFormPopup(date, jsEvent, view) {
    ...
    // Find the date corresponding to the clicked time slot
    var clickedDate = $(this).closest('.fc-row').data('date');

    // If clickedDate is not found, use the current date of the calendar as a fallback
    if (!clickedDate) {
        clickedDate = $('#calendar').fullCalendar('getDate').format('YYYY-MM-DD');
    }

    // Combine the date with the time
    var slotDateTime = clickedDate + ' ' + $(this).closest('[data-time]').data('time');

    // Format the date and time for the alert display
    var formattedDisplayDateTime = moment(slotDateTime, 'YYYY-MM-DD HH:mm').format('dddd Do MMM, YYYY [at] h:mm A');

    // Format the date and time for the input field
    var formattedInputDateTime = moment(slotDateTime, 'YYYY-MM-DD HH:mm').format('YYYY-MM-DD HH:mm:ss');

    // Display an alert with the selected date and time
    //alert("Selected Date and Time: " + formattedDisplayDateTime);

    // Assign the formatted date and time to the hidden input field
    $('#appointment_date_time_database').val(formattedInputDateTime);

    // Display the formatted date and time in the <span> element
    $('.selected-date-time').text(formattedDisplayDateTime);

    // Open the appointment form popup with the selected date and time
    openAppointmentFormPopup(formattedDisplayDateTime, null);
});
...
<script>
  ...
  <div class="modal fade" id="appointmentFormModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" style="left:-420px;width:1500px;height:1000px">
    <div class="modal-dialog" role="document">
      <div class="modal-content" style="margin-top:-40px;">
     
        <!-- Your appointment form HTML goes here -->
        <!-- Replace this with your actual form HTML -->
        <div class="modal-body" style="width:1000px;height:850px;position: static !important;">
          <!-- Your form content goes here -->
          <h3>Appointment Form</h3>
          <div id="appointment-form" style="left: 0px;">
            <div class="appointment-form-wrapper" style="width: 1000px;">
              <form class="form-horizontal" method="POST" action="drspine_appointment_insertdb.php" id="appointment-booking-form">
                ...
                <div class="appointment-form-footer">
                  ...
                  <div class="appointment-form-footer-right">
                    ...
                    <div class="button-wrapper" style="text-align: right;">
                      <div class="button-save" style="display: inline-block;" onclick="submitForm()">
                     ...
                     <!-- <button type="button" class="btn btn-secondary" onclick="closeForm()">Close</button> -->
                      <button style="background: red; border: none; color: white; font-family: ' UI Text '; text-: none; padding: 5px 15px font-size: 14px; border-radius: 4px; margin-left: 10px;" type="button" onclick="closeForm()">Close</button>
...
/*
    ...
    <script>
    function submitForm() {
        document.getElementById('appointment-booking-form').submit();
    }
</script>
  <!-- <script>
