...
eventRender: function (event, element) {
    element.popover({
        title: event.title,
        content: '<div id="popover-content">Loading...</div>',
        html: true,
        placement: 'top',
        trigger: 'hover',
        container: 'body'
    ...
    element.on('shown.bs.popover', function () {
        // Fetch appointment details using AJAX when popover is shown
        $.ajax({
            url: 'get_appointment_details.php', // Replace with your script to fetch appointment details
            type: 'GET',
            data: { id: event.id },
            success: function (response) {
            ...
            error: function () {
                $('#popover-content').html('Failed to load appointment details.');
            }
        });
    });
},
...
<script>
    function searchPatients(search) {
        ...
        const searchResultsContainer = document.getElementById('search-results'); // New container for search results
      ...
      //  searchResultsContainer.innerHTML = '';
        ...
        if (search.trim() !== '') {
            fetch(`search_drop_down_single_pateint.php?search=${search}`)
                ...
                .then(data => {
                    if (data.success) {
                        // If the response indicates success, populate the search results container
                        const patients = data.data;

                        // Inside your loop where you iterate over patients
                        for (let i = 0; i < patients.length; i++) {
                            ...
                            // Populate the result item with patient information
                            resultItem.textContent = `${patient.patient_first_name} ${patient.patient_last_name}`;

                            // Add an event listener to handle result item selection (if needed)

                            // Append the result item to the search results container
                            searchResultsContainer.appendChild(resultItem);
                        }

                        // Clear the suggestions container (if needed)
