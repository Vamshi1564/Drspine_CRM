...
<link href="https://cdn.kivihealth.com/jqueryui/1.11.4/jquery-ui.min.css" rel="stylesheet">
 ...
 -->
  ...
  <style>
    ...
    .today-session-list::-webkit-scrollbar {
      width: 12px;
      /* Width of the scrollbar */
    }

    .today-session-list::-webkit-scrollbar-track {
      background: #f1f1f1;
      /* Track color */
    }

    .today-session-list::-webkit-scrollbar-thumb {
      background: #888;
      /* Handle color */
    }

    .today-session-list::-webkit-scrollbar-thumb:hover {
      background: #555;
    ...
    /* For Webkit (Chrome, Safari, etc.) */
    .today-appointment-list::-webkit-scrollbar {
      width: 12px;
      /* Width of the scrollbar */
    }

    .today-appointment-list::-webkit-scrollbar-track {
      background: #f1f1f1;
      /* Track color */
    }

    .today-appointment-list::-webkit-scrollbar-thumb {
      background: #888;
      /* Handle color */
    }

    .today-appointment-list::-webkit-scrollbar-thumb:hover {
      background: #555;
      /* Handle color on hover */
    }
...
<body class="hold-transition sidebar-mini layout-fixed">
  ...
  <div id="email_loader" style="display:none;" class="email_loader">
    <i class="fas fa-spinner fa-spin"></i>
    Please Wait while booking appointment...
  </div>
          ...
          <div class="row">
            <div class="col-md-12">
              ...
              <div class="card card-primary">
                ...
                <div class="card-header" style="padding: 5px 5px 5px;height:45px;">
                ...
                <script>
                  ...
                  function fetchAndInitializeCalendar(branchName, view) {
                    // alert("fetchAndInitializeCalendar");
                    var viewOption;
                    if (view === 'today') {
                    ...
                    $.ajax({
                      url: 'getAPI_combined_details.php', // Replace with the correct API endpoint for both appointments and sessions
                      type: 'GET',
                      data: {
                        branch_name: branchName
                      },
                      success: function(data) {
                        // Assuming your API returns an array of appointment and session objects
                  ...
                  <script>
                    ...
                    function displaySessions(data) {
                      data.forEach(function(session) {
                        ...
                        sessionListItem.innerHTML = `<span class="tip"></span>
        <div class="session-list-item-wrapper">
          <div class="session-list-item-left">
            ...
            <div class="session-patient-name-time-wrapper">
              ...
              <div class="patient-time-wrapper" style="margin-top: 5px;">
                <span class="session-time">${session.time_from.substring(0, 5)}</span>
              </div>
                  ...
                  <div id="sessionDetailsModal" class="modal">
                    <div class="modal-content">
                      <div id="session_details_modal" class="session-details-modal">
                        ...
                        <p style="color: blue" id="session_no" hidden></p>
                        <input type="hidden" id="session-modal-id" name="session-modal-id">

                        <div class="col-md-12">

                          <p id="sessionTitle" hidden></p>
                          <p id="sessionModalID" hidden></p>
                          <p id="appointmentNumber"></p>
                          <p id="patientId" hidden></p>
                          <p id="branchName"></p>
                          <p id="dateTime"></p>
                          <p id="treatment"></p>
                          <p id="doctor"></p>
  ...
  <script>
    ...
    function fetchAppointmentData(branchName) {
      ...
      $.ajax({
        ...
        success: function(data) {
          ...
          // var startDateTime = moment(session.date_time);
          ...
          eventsData = eventsData.concat(data.sessions.map(function(session) {
            ...
            var endDateTime = moment(endDateTimeString, "YYYY-MM-DD HH:mm");
    ...
    $(document).ready(function() {
      ...
      function openAppointmentFormPopup(slotDate, slotTime, jsEvent) {
        // Reset the form every time the modal is opened
        document.getElementById('appointmentBookingForm').reset();
        ...
        var selectedDateTime = slotDate + ' at ' + slotTime;

        // Set the selected date and time in your appointment-date-picker
        showDateTimePicker(selectedDateTime);

      }


      // Add event listener to modal for the 'show' event if needed
      $('#appointmentFormModal').on('show.bs.modal', function(e) {
        document.getElementById('appointmentBookingForm').reset();
      });
      ...
      $('#calendar').on('click', '.fc-slats td', function() {
        // Get the element that was clicked
        var slotDate = $('#calendar').fullCalendar('getDate').format('dddd D MMM, YYYY'); // Get the current date of the calendar

        // Find the closest parent with 'data-time' attribute and get its value
        var slotTime = $(this).closest('[data-time]').data('time');

        // Display an alert with the selected time
        // alert("Selected Time: " + slotTime);

        // Open the appointment form popup with the selected date and time
        openAppointmentFormPopup(slotDate, slotTime, null);
      });
  ...
  <div class="modal fade" id="appointmentFormModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" style="width:1500px;height:800px">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <div class="modal-body" style="width:1000px;height:1000px">
          ...
          <div id="appointment-form" style="left: 0px;">
            <div class="appointment-form-wrapper" style="width: 1000px;">
              <form id="appointmentBookingForm" class="form-horizontal" method="POST" action="drspine_appointment_insertdb.php">
                ...
                <div class="appointment-form-header">
                  ...
                  <div class="appointment-form-header-right" style="display: flex;">
                    ...
                    <script>
                      ...
                      function setSelectedDateTime(dateTime) {
                        ...
                        selectedDateTimeSpan.textContent = dayOfWeek + ' ' + day + 'th ' + monthName + ', ' + year + ' at ' + displayHours + ':' + minutes + ' ' + ampm;
                      ...
                      function updateSelectedDateTime() {
                        ...
                        if (!isNaN(selectedDateTime.getTime())) {
                          ...
                          // Update the appointment-date-picker with the selected time
                          var appointmentDatePicker = document.getElementById("appointment-date-picker");
                          appointmentDatePicker.querySelector(".selected-date-time").textContent = selectedDateTimeSpan.textContent;
                        }
                      }
                      ...
                      // Function to show the datetime picker with a selected date and time
                      function showDateTimePicker(selectedDateTime) {
                        const currentDatetimeISO = new Date().toISOString().slice(0, -8);
                        alert(SelectedDateTime);
                        datetimePicker.min = currentDatetimeISO;
                        appointmentTimeInput.value = selectedDateTime; // Set the selected date and time directly
                        selectedDateTimeSpan.textContent = selectedDateTime;
                        datetimePicker.style.display = "block";
                      }
                    </script>
                      ...
                      <div class="form-group col-sm-9 appointment-form-patient-name-fields">
                        <div class="row col-sm-6 col-lg-6">
                          <div class="react-autosuggest__container">
                            ...
                            <input type="text" autocomplete="off" role="combobox" aria-autocomplete="list" aria-owns="react-autowhatever-1" aria-expanded="false" class="form-control required-field" name="patient_first_name" placeholder="First name" id="appointment-form-patient-fname" value="" oninput="if (this.value.length >= 4) searchPatients(this.value)" onclick="handleInputClick(event)">
                        ...
                        <script>
                          function searchPatients(search) {
                            ...
                            const appointmentFormPatientDob = document.getElementById('appointment-form-patient-dob');
                            const appointmentFormPatientAddress = document.getElementById('appointment-form-patient-address');
                            ...
                            inputFieldId.value = '';
                            appointmentFormPatientDob.value = '';
                            appointmentFormPatientAddress.value = '';
                                    ...
                                    for (let i = 0; i < patients.length; i++) {
                                      ...
                                      for (let i = 0; i < patients.length; i++) {
                                        ...
                                        listItem.addEventListener('click', () => {
                                          ...
                                          appointmentFormPatientDob.value = patient.date_of_birth;
                                          appointmentFormPatientAddress.value = patient.patient_address;
                ...
                <div class="appointment-form-footer">
                  ...
                  <div class="appointment-form-footer-right">
                    ...
                    <div class="button-wrapper pt-2" style="text-align: right;">
                      <div class="button-save" style="display: inline-block;">
                        <button type="submit">Book Appointment</button>
                      </div>

                      <!-- <div class="modal-footer" style="float:right;margin-left:40%;">
                        <button type="button" class="btn btn-danger" data-dismiss="modal">Close</button>
                        <div class="button-cancel" style="display: flex;"><button type="button" class="btn btn-secondary" data-dismiss="modal">Reset Form</button></div>
                      </div> -->

                      <div class="modal-footer" style="float:start;">
                        <div style="display: flex; justify-content: space-between; width:200px;height:40px;">
                          <button type="button" class="btn btn-danger" data-dismiss="modal">Close</button>
  ...
  <script>
    function showLoader() {
      ...
      document.getElementById('email_loader').style.display = 'block';

      setTimeout(function() {
        // Refresh the page after 5 seconds
        location.reload();
      }, 120000);
      // Allow the loader to be shown before the form is submitted
      setTimeout(function() {
        // Optional: Submit the form
        document.getElementById('appointment-form').submit();

        // Wait for some time (e.g., 3 seconds) before hiding the loader
        setTimeout(function() {
          // Hide the loader after the additional delay
