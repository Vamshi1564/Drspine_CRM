...
if (isset($_POST['download_month'])) {
    ...
    // Fetch patient data based on the selected month
    $sql = "SELECT * FROM `drspine_appointment` WHERE `appointment_date_time` BETWEEN '$start_month 00:00:00' AND '$end_month 23:59:59'";
...
// Generate the file name based on the selected month and year
//$file_name = "patient_data_monthwise_{$year}_{$month}.csv";
$formatted_month_year = date("F-Y", strtotime("$year-$month-01"));
    $file_name = "patient_data_monthwise_{$formatted_month_year}.csv";
