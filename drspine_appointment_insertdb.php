...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (($_POST['patient_id']) !== "new_patient") {
      // echo '<script>alert("'.$_POST['patient_id'].'") </script>';
...
if ($result && $result->num_rows > 0) {
    ...
    $sql = "SELECT MAX(appointment_number) AS max_appointment_number FROM drspine_appointment WHERE patient_id = ?";
    ...
    if ($result->num_rows === 1) {
        ...
        // echo '<script>alert("For old patient Appointment no :'. $next_appointment_number .' ");</script>';
    } else {
        // No existing appointments for this patient, start with "appointment_no_1"
        $next_appointment_number = "appointment_no_1";
        // echo '<script>alert("For New patient Appointment no :'. $next_appointment_number .' not found");</script>';
    }
  } 
} 
else {
  // echo '<script>alert("'.$_POST['patient_id'].'") </script>';
      // If patient_id is not set in the POST data, fetch the maximum patient_id from the database and increment it by 1
      // echo '<script>alert("new patient") </script>';
      $sql = "SELECT MAX(patient_id) AS max_patient_id FROM drspine_appointment";
      $result = mysqli_query($conn, $sql); // Replace $your_db_connection with your database connection variable
      $next_appointment_number = "appointment_no_1";
      // echo '<script>alert("For New patient Appointment no :'. $next_appointment_number .' not found");</script>';
      if ($result) {
          ...
          // $patient_id = 23000; // Default value
      }
  }
...
$date_of_birth = isset($_POST['date_of_birth']) && !empty($_POST['date_of_birth']) ? $_POST['date_of_birth'] : "0000-00-00";
...
$no_of_sessions_plans = isset($_POST['no_of_sessions_plans']) ? $_POST['no_of_sessions_plans'] : null;
...
// $sql = "INSERT INTO drspine_appointment (patient_id, appointment_type, appointment_date_time, duration_minutes, doctor_name, patient_first_name, patient_middle_name, patient_last_name, patient_profile_picture, contact_no, email_address, gender, date_of_birth, referred_by, refer_patient_type, refer_patient_name, location_city, location_area, patient_address, reason_for_appointment, case_id, medical_history, procedures, groups, blood_group, id_type, id_number, nh_id, other_history, branch_name) 
// VALUES ('$patient_id', '$appointment_type', '$appointment_date_time', '$duration_minutes', '$doctor_name', '$patient_first_name', '$patient_middle_name', '$patient_last_name', '$patient_profile_picture', '$contact_no', '$email_address', '$gender', '$date_of_birth', '$referred_by', '$refer_patient_type', '$refer_patient_name', '$location_city', '$location_area', '$patient_address', '$reason_for_appointment', '$case_id', '$medical_history', '$procedures', '$groups', '$blood_group', '$id_type', '$id_number', '$nh_id', '$other_history', '$branch_name')";
// Display the SQL query in the browser's console
$query = "SET @new_no_of_appointments = (
  SELECT IFNULL(MAX(no_of_booked_appointments_patient_id), 0) + 1
  FROM drspine_appointment
  WHERE patient_id = $patient_id
);";
echo "<script>console.log('SQL Query: " . mysqli_real_escape_string($conn, $query) . "');</script>";
if ($conn->query($query) === TRUE) {
  // Now, insert the new appointment

  $insertQuery = "INSERT INTO drspine_appointment (patient_id,status, appointment_number, appointment_type, appointment_date_time, duration_minutes, doctor_name, patient_first_name, patient_middle_name, patient_last_name, patient_profile_picture, contact_no, email_address, gender, date_of_birth, age, referred_by, refer_patient_type, refer_patient_name, location_city, location_area, patient_address, reason_for_appointment, case_id, medical_history, procedures, groups, blood_group, id_type, id_number, nh_id, other_history, branch_name, no_of_booked_appointments_patient_id, occupation, pincode, treatment, source, no_of_sessions_plans) 
  VALUES ($patient_id, 'scheduled','$next_appointment_number', '$appointment_type', '$appointment_date_time', '$duration_minutes', '$doctor_name', '$patient_first_name', '$patient_middle_name', '$patient_last_name', '$patient_profile_picture', '$contact_no', '$email_address', '$gender', '$date_of_birth', $age, '$referred_by', '$refer_patient_type', '$refer_patient_name', '$location_city', '$location_area', '$patient_address', '$reason_for_appointment', '$case_id', '$medical_history', '$procedures', '$groups', '$blood_group', '$id_type', '$id_number', '$nh_id', '$other_history', '$branch_name', @new_no_of_appointments, '$occupation', $pincode, '$treatment', '$source', '$no_of_sessions_plans');";
echo "<script>console.log('SQL Query: " . mysqli_real_escape_string($conn, $insertQuery) . "');</script>";

if ($conn->query($insertQuery) === TRUE) {

 
if ($email_address == null) {
  echo "<script>console.log('email is snull');</script>";
...
exit;
  ...
  echo "<script>console.log('before sending mail');</script>";
 //      echo '<script>alert("before sending mail");</script>';
...
// if ($conn->query($sql) === TRUE) {
  ...
  // Date and time format with colons
  

  try {
    // Ensure $appointment_date_time is in the correct format (e.g., "Y-m-d H:i:s")
    ...
    if ($datetime !== false) {
        ...
        $appointment_date_time = $datetime->format("l jS M, Y \a\\t g:i A");
    } else {
