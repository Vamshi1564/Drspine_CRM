...
<!-- Then include Select2 JS (since it depends on jQuery) -->
    ...
    if ($result->num_rows > 0) {
        ...
        <div class="row border-dark border-right border-left border-top border-bottom" id="box_loop_1">
            <div class="col-md-3 p-3 border-right border-left border-bottom">
              ...
              <select name="product_choice" id="product_choice" class="form-control select2">
...
price = parseFloat($('#price').val());
 gst = parseFloat($('#gst').val());
      ...
      //(in_ex_gst == "Exclusive of GST")
      //
        //ice = parseFloat($('#price').val());
     //
      //se if(in_ex_gst == "Inclusive of GST")
      //
        //ice = parseFloat($('#net_price').val());
      //lse{
        //ice = parseFloat($('#price').val());
     //
    ...
    // Adjust price based on GST inclusion
    if (in_ex_gst === "Inclusive of GST") {
        // If price is inclusive of GST, subtract the GST
        price = parseFloat((price / (1 + gst / 100)).toFixed(2));
    } else if (in_ex_gst === "Exclusive of GST") {
        // If price is exclusive of GST, add the GST
        price = parseFloat((price * (1 + gst / 100)).toFixed(2));
    }
    ...
    var gstamt = 0;
    if (gst > 0) {
        gstamt = parseFloat((totalWithoutDiscount * (gst / 100)).toFixed(2));
    }

    // Calculate the final total
    var total = parseFloat((totalBeforeGST + gstamt).toFixed(2));

    // Display or use the calculated values as needed
    console.log('GST Amount:', gstamt);
   ...
   // }
     ...
     //var totalWithoutDiscount = qty * price;
//r discountAmount = (totalWithoutDiscount * discount) / 100;
//r totalBeforeGST = totalWithoutDiscount - discountAmount;
...
//			var total = ((totalBeforeGST + gstamt).toFixed(2));
      ...
      $('#product_choice').val('select packages').trigger('change');
    ...
    function calc_total() {
      ...
      $('#item-list tbody tr').each(function() {
       ...
       $('#item-list tbody tr').each(function() {
        var tr = $(this)
        ptotal += (parseFloat(tr.find('[name="priceval[]"]').val())) * (parseFloat(tr.find('[name="qtyvalue[]"]').val()));
      ...
      $('[name="sub_total"]').val(ptotal)
      $('#sub_total').text(parseFloat(ptotal).toLocaleString('en-US'))
  ...
  // Initialize Select2 on the package dropdown
  $('.select2').select2();

  // Fetch packages when the centre type changes
  $("#center_type").change(function() {
    var centerType = $(this).val();
    fetchPackages(centerType);
  });

  // Function to fetch packages
  function fetchPackages(centerType) {
    $.ajax({
      type: 'POST',
      url: 'fetch_packages.php', // Your PHP script to fetch packages
      data: { center_type: centerType },
      success: function(data) {
        $('#product_choice').html(data); // Update the options of the package dropdown
        $('#product_choice').select2(); // Re-initialize Select2
      }
    });
  }
