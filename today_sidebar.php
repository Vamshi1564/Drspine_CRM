...
//   }
  ...
  eventDrop: function(event, delta, revertFunc) {
    ...
    <div>
        <div class="calendar-right-side-header"><span class="header-title">Today's Schedule</span></div>
        <div class="appointment-state-wrapper">
            <div class="appointment-state state-schedule">
                <div class="appointment-state-number-wrapper"><span></span></div>
                <div class="appointment-state-detail-wrapper"><span>Schedule</span></div>
            </div>
            <div class="appointment-state state-waiting">
                <div class="appointment-state-number-wrapper"><span>0</span></div>
        ...
        <div class="today-appointment-list _webkit_scroll_bar">
                ...
                <div class="appointment-list-item-wrapper">
                    <div class="appointment-list-item-left">
                        ...
                        <div class="patient-name-time-wrapper">
                            <div class="patient-name-wrapper">
                                <span class="patient-name">Bharat Chandra</span></div>
                    ...
                    <div class="appointment-list-item-extra">
                        <div class="status-button">
                            <span class="state-info status-text status-schedule">Scheduled</span>
                            <span class="state-info status-text status-waiting">Waiting</span>
                        </div>
                        <div class="status-loading hidden"><span class="state-info">Loading..</span>
                    </div>
                    <span class="material-icons" data-toggle="tooltip" data-placement="bottom" title="Ask for Review">star_half</span>
                </div>
            </div>
        </div>
                    <!-- <div class="appointment-list-item"><span class="tip"></span>
                    <div class="appointment-list-item-wrapper"> -->
...
<script>
  ...
  function fetchAppointments() {
    ...
    xhr.open('GET', 'getAPI_today_appointment_details.php', true);

    xhr.onload = function() {
      if (xhr.status === 200) {
        var data = JSON.parse(xhr.responseText);
        // Process the data and dynamically display it in the calendar-right-sidebar
        var appointmentStateWrapper = document.querySelector('.appointment-state-wrapper');

        data.forEach(function(appointment) {
          var appointmentDiv = document.createElement('div');
          appointmentDiv.classList.add('appointment-state');
          appointmentDiv.classList.add('state-schedule'); // You may need to adjust this class
          appointmentDiv.innerHTML = `
            <div class="appointment-state-number-wrapper"><span>${appointment.patient_name}</span></div>
            <div class="appointment-state-detail-wrapper"><span>Schedule</span></div>
          `;
          appointmentStateWrapper.appendChild(appointmentDiv);
        });
      } else {
  ...
  // Call the fetchAppointments function to load data
  fetchAppointments();
</script>
