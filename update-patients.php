...
if (isset($_POST['submit'])) {
    ...
    $selected_treatments = $_POST['selected_treatments'];

    // Update the patient's information in the database
    $update_sql = "UPDATE patients SET pname=?, gender=?, dob=?, occupation=?, age=?, email=?, date=?, contact_number=?, location=?, city=?, source=?, session_plans=?, treatment=? WHERE id=?";
    $update_stmt = $conn->prepare($update_sql);
    $update_stmt->bind_param("ssssissssssssii", $pname, $gender, $dob, $occupation, $age, $email, $date, $contact_number, $location, $city, $source, $session_plans, $selected_treatments, $id);
    ...
    if ($update_result) {
        ...
        echo '<script>';
        echo 'setTimeout(function(){ window.location.href = "view-patients.php"; }, 2000);'; // Redirect after 2 seconds
        echo '</script>';
    }
}
?>
...
<form action="update-patients.php" method="POST" enctype="multipart/form-data">
                <div class="card-body">
                <h3>Basic info</h3><br>
                <div class="row">
                <div class="form-group col-md-4">
                    <label for="id">Patient ID</label>
                    <input type="number" class="form-control" id="id" name="id" value="<?php echo $patient['id']; ?>"   required>
                  </div>
                  <div class="form-group col-md-4">
